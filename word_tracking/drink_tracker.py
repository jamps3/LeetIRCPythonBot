...
class DrinkTracker:
    ...
    def __init__(self, data_manager: DataManager):
        ...
        self.drink_words = {
            "krak", "kr1k", "kr0k", "narsk", "parsk", 
            "tlup", "marsk", "tsup", "plop", "tsirp"
        }
    ...
    def process_message(self, server: str, nick: str, text: str) -> List[Tuple[str, str]]:
        """
    ...
    def _record_drink_word(self, server: str, nick: str, drink_word: str, specific_drink: str):
        """
    ...
    def get_server_stats(self, server: str) -> Dict[str, Any]:
        ...
        if server not in data.get("servers", {}):
            return {"server": server, "total_users": 0, "total_drink_words": 0, "top_users": []}
        ...
        # Get top users
        top_users = sorted(
            [(nick, user.get("total_drink_words", 0)) for nick, user in server_data.items()],
            key=lambda x: x[1],
            reverse=True
        )[:10]
    ...
    def get_global_stats(self) -> Dict[str, Any]:
        ...
        for server_name, server_data in data.get("servers", {}).items():
            ...
            server_total = sum(user.get("total_drink_words", 0) for user in nicks.values())
        ...
        global_top_users = sorted(global_top_users, key=lambda x: x["total"], reverse=True)[:10]
    ...
    def search_drink_word(self, drink_word: str) -> Dict[str, Any]:
        ...
        # Sort users by total
        results["users"] = sorted(results["users"], key=lambda x: x["total"], reverse=True)
    ...
    def get_user_top_drinks(self, server: str, nick: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        for drink_word, data in drink_words.items():
            drink_list.append({
                ...
                "total": data["total"],
                "drinks": data["drinks"],
                "most_common_drink": max(data["drinks"].items(), key=lambda x: x[1])[0] if data["drinks"] else "unspecified"
            })
