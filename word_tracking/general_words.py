...
class GeneralWords:
    ...
    def process_message(self, server: str, nick: str, text: str, target: str = None):
        ...
        if self.lemmatizer:
            try:
                self.lemmatizer.process_message(text, server_name=server, source_id=target or nick)
            except Exception as e:
                print(f"Error in lemmatizer processing: {e}")
    
    def _update_word_stats(self, server: str, nick: str, words: List[str], target: str = None):
        """
    ...
    def get_user_top_words(self, server: str, nick: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def get_server_stats(self, server: str) -> Dict[str, Any]:
        ...
        if server not in data.get("servers", {}):
            return {"server": server, "total_users": 0, "total_words": 0, "top_users": [], "top_words": []}
    ...
    def search_word(self, word: str) -> Dict[str, Any]:
        ...
        # Sort users by count
        results["users"] = sorted(results["users"], key=lambda x: x["count"], reverse=True)
        
        return results
    
    def get_leaderboard(self, server: str = None, limit: int = 10) -> List[Dict[str, Any]]:
        """
