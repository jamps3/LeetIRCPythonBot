...
def test_console_command():
    ...
    try:
        ...
        electricity_service = create_electricity_service(electricity_api_key) if electricity_api_key else None
        if electricity_service:
            print("⚡ Electricity service initialized")
        
        youtube_api_key = os.getenv('YOUTUBE_API_KEY')
        youtube_service = create_youtube_service(youtube_api_key) if youtube_api_key else None
        if youtube_service:
        ...
        bot_functions = {
            ...
            'get_crypto_price': lambda coin, currency='eur': crypto_service.get_crypto_price(coin, currency)['price'] if not crypto_service.get_crypto_price(coin, currency).get('error') else 'N/A',
