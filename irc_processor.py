...
class MessageContext:
    ...
    def is_bot_mention(self) -> bool:
        ...
        return text_lower.startswith(f"{bot_name}:") or text_lower.startswith(
            f"{bot_name},"
        )
    ...
    def is_private_to_bot(self) -> bool:
        ...
        return (
            self.irc_message.target
            and self.irc_message.target.lower() == self.config.name.lower()
        )
...
class IRCMessageProcessor:
    ...
    def __init__(self, irc_client: IRCClient, bot_functions: Dict[str, Any]):
        ...
        self.irc_client.add_message_handler(
            IRCMessageType.PRIVMSG, self._handle_privmsg
        )
    ...
    def _process_message(self, irc_message: IRCMessage):
        ...
        for processor in self._processors:
            try:
                ...
                self._log(
                    f"Message processor error in {processor.__name__}: {e}", "ERROR"
                )

    def _process_ignore_self(self, context: MessageContext) -> bool:
        """Ignore messages from the bot itself."""
        if (
            context.irc_message.sender
            and context.irc_message.sender.lower() == self.config.name.lower()
        ):
    ...
    async def _process_commands_async(self, context: MessageContext) -> bool:
        ...
        try:
            ...
            response = await process_command_message(
                context.irc_message.text, cmd_context, enhanced_bot_functions
            )
            ...
            if response and response.should_respond and response.message:
                ...
                if response.split_long_messages:
                    # Split long messages
                    split_func = context.bot_functions.get(
    ...
    def _process_ai_chat(self, context: MessageContext) -> bool:
        ...
        try:
            ...
            response = chat_with_gpt(
                context.irc_message.text, context.irc_message.sender
            )
            if response:
                # Determine target
                target = (
                    context.irc_message.sender
                    if context.is_private_to_bot
                    else context.irc_message.target
                )
                ...
                if wrap_func:
                    parts = wrap_func(
                        response, reply_target=target, max_lines=5, placeholder="..."
                    )
                    for part in parts:
                        self._send_response(target, part)
                else:
            ...
            if match:
                ...
                if load_leet_winners and save_leet_winners:
                    ...
                    for category, winner in zip(
                        ["ensimmäinen", "viimeinen", "multileet"],
                        [first, last, multileet],
                    ):
...
def create_message_processor(
    irc_client: IRCClient, bot_functions: Dict[str, Any]
) -> IRCMessageProcessor:
    """
