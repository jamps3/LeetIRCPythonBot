...
from main import (
    # Core functions
    load, save, update_kraks, load_leet_winners, save_leet_winners,
    # Bot functions
    send_weather, send_electricity_price, fetch_title, handle_ipfs_command,
    chat_with_gpt, wrap_irc_message_utf8_bytes, split_message_intelligently,
    get_crypto_price, measure_latency, count_kraks,
    # Advanced functions
    search_youtube, get_eurojackpot_numbers, send_scheduled_message,
    format_counts, lookup,
    # Configuration
    DRINK_WORDS, EKAVIKA_FILE, BOT_VERSION,
    # Services
    lemmat, subscriptions
)
...
class RefactoredBot:
    ...
    def __init__(self, server_name: str = "SERVER1", nickname: str = None, log_level: str = "INFO"):
        """
        Initialize the bot.
        ...
        self.message_processor = create_message_processor(self.irc_client, self.bot_functions)
                ...
                try:
                    ...
                    if user_input.startswith('!'):
                        ...
                        try:
                            ...
                            enhanced_process_console_command(user_input, self.bot_functions)
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if not self.stop_event.is_set():
                    self._log("Connection lost, reconnecting in 30 seconds...", "WARNING")
                    for _ in range(30):
                        if self.stop_event.is_set():
                            break
                        time.sleep(1)
...
def main():
    ...
    parser.add_argument(
        "-n", "--nick",
        help="Bot nickname (uses config default if not specified)"
    )
    parser.add_argument(
        "--test-config",
        action="store_true",
