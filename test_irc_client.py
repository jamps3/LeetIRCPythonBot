...
def test_irc_parsing():
    ...
    for raw_msg in test_messages:
        ...
        if parsed:
            print(f"✓ Parsed: {parsed.type.value} from {parsed.sender} to {parsed.target}")
...
def test_irc_client_configuration():
    ...
    try:
        client = create_irc_client("SERVER1", "testbot")
        print(f"✓ Created client for server: {client.server_config.host}:{client.server_config.port}")
...
def test_message_processor():
    ...
    bot_functions = {
        ...
        'DRINK_WORDS': {'krak': 0},
        'count_kraks': lambda word, beverage: print(f"Counted drink: {word} ({beverage})"),
    }
    ...
    try:
        ...
        test_message = client.parse_message(":testuser!user@host.com PRIVMSG #test :Hello world test")
        if test_message:
            processor._process_message(test_message)
            print("✓ Processed regular message")
        
        # Test command processing
        cmd_message = client.parse_message(":testuser!user@host.com PRIVMSG #test :!help")
        if cmd_message:
