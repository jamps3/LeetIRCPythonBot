...
class CommandHandler(ABC):
    ...
    @abstractmethod
    async def execute(self, context: CommandContext, bot_functions: Dict[str, Any]) -> CommandResponse:
...
class FunctionCommandHandler(CommandHandler):
    ...
    def _validate_function(self):
        ...
        if len(params) < 2:
            raise ValueError(f"Command function {self.func.__name__} must accept at least (context, bot_functions)")
...
class CommandRegistry:
    ...
    def get_command_names(self, include_aliases: bool = False, 
        ...
        for name, handler in self._commands.items():
            ...
            # Apply filters
            if command_type and info.command_type != command_type and info.command_type != CommandType.ALL:
                continue
    ...
    def get_commands_info(self, command_type: Optional[CommandType] = None,
                         scope: Optional[CommandScope] = None,
                         include_hidden: bool = False) -> List[CommandInfo]:
    ...
    async def execute_command(self, command_name: str, context: CommandContext, 
                            bot_functions: Dict[str, Any]) -> Optional[CommandResponse]:
    ...
    def generate_help(self, command_type: Optional[CommandType] = None,
                     scope: Optional[CommandScope] = None,
                     specific_command: Optional[str] = None) -> str:
...
def parse_command_message(message: str, command_prefix: str = "!") -> tuple[Optional[str], List[str], str]:
    """
...
async def process_command_message(message: str, context: CommandContext, 
                                bot_functions: Dict[str, Any],
                                command_prefix: str = "!") -> Optional[CommandResponse]:
    """
