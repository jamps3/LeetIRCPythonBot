...
def test_irc_message_parsing():
    ...
    try:
        ...
        test_cases = [
            (":nick!user@host PRIVMSG #channel :Hello world", IRCMessageType.PRIVMSG, "#channel", "Hello world"),
            (":nick!user@host JOIN #channel", IRCMessageType.JOIN, "#channel", None),
            ("PING :server.example.com", IRCMessageType.PING, None, "server.example.com"),
            (":server 001 testbot :Welcome", IRCMessageType.NUMERIC, None, "Welcome"),
            (":nick!user@host PRIVMSG testbot :Private message", IRCMessageType.PRIVMSG, "testbot", "Private message"),
        ]
...
def test_irc_connection_states():
    ...
    try:
        ...
        assert client.connection_info.state == IRCConnectionState.DISCONNECTED, "Initial state should be disconnected"
        
        # Test connection info properties
        assert client.connection_info.uptime is None, "Uptime should be None when disconnected"
...
def test_irc_handler_system():
    ...
    try:
        ...
        assert len(client._message_handlers.get(IRCMessageType.PRIVMSG, [])) == 0, "Handler should be removed"
...
def test_irc_channel_management():
    ...
    try:
        ...
        assert isinstance(client.connection_info.channels, list), "Channels should be a list"
        ...
        try:
            ...
            assert "#test" in client.connection_info.channels, "Channel should be added to list"
        except:
        ...
        try:
            ...
            assert "#test" not in client.connection_info.channels, "Channel should be removed from list"
        except:
            # Network error is expected in test environment
            pass
...
def register_irc_client_tests(runner: TestRunner):
    ...
    suite = TestSuite(
        name="IRC_Client",
        description="Tests for IRC client functionality",
        tests=tests
    )
