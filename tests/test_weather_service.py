...
def test_weather_service_creation():
    ...
    try:
        ...
        assert service.base_url == "http://api.openweathermap.org/data/2.5", "Base URL should be set"
        
        # Test factory function
        service2 = create_weather_service("another_key")
        assert service2.api_key == "another_key", "Factory should set API key"
        assert isinstance(service2, WeatherService), "Factory should return WeatherService instance"
...
def test_wind_direction_calculation():
    ...
    try:
        ...
        for degrees, expected_emoji in test_cases:
            ...
            assert result == expected_emoji, f"Wind direction for {degrees}° should be {expected_emoji}, got {result}"
...
def test_weather_emoji_mapping():
    ...
    try:
        ...
        for condition, expected_emoji in test_cases:
            ...
            assert result == expected_emoji, f"Weather emoji for {condition} should be {expected_emoji}, got {result}"
...
def test_pressure_analysis():
    ...
    try:
        ...
        for pressure, expected_visual in test_cases:
            ...
            assert result['visual'] == expected_visual, f"Pressure visual for {pressure} hPa should be {expected_visual}, got {result['visual']}"
...
def test_precipitation_handling():
    ...
    try:
        ...
        assert no_precip_result.endswith("."), "Should end with period when no precipitation"
