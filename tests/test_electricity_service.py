        ...
        for parts_list in test_cases:
            with self.subTest(parts=parts_list):
                try:
                    ...
                    self.assertTrue(True, f"Successfully handled list input: {parts_list}")
                except AttributeError as e:
                    if "'list' object has no attribute 'split'" in str(e):
                        self.fail(f"Function failed to handle list input {parts_list}: {e}")
                    else:
        ...
        for text_input in test_cases:
            with self.subTest(text=text_input):
                try:
                    ...
                    self.assertTrue(True, f"Successfully handled string input: '{text_input}'")
                except Exception as e:
