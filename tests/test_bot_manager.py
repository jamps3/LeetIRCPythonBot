...
def test_url_blacklist_and_banned_title(monkeypatch, manager):
    monkeypatch.setenv("TITLE_BLACKLIST_DOMAINS", "example.com")
    monkeypatch.setenv("TITLE_BLACKLIST_EXTENSIONS", ".jpg,.mp4")
    assert manager._is_url_blacklisted("http://example.com/page") is True
    assert manager._is_url_blacklisted("http://site.com/file.jpg") is True
    assert manager._is_url_blacklisted("http://site.com/page") is False

    monkeypatch.setenv("TITLE_BANNED_TEXTS", "forbidden;not found")
    assert manager._is_title_banned("This is Forbidden Area") is True
    assert manager._is_title_banned("All good") is False
