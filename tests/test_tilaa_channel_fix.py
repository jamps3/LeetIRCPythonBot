...
class TestTilaaChannelFix(unittest.TestCase):
    ...
    def test_tilaa_list_shows_channel_subscriptions(self):
        ...
        self.assertNotIn(
            "ei ole tilannut mitään",
            full_response,
            "Should show actual subscriptions, not empty message",
        )
    ...
    def test_tilaa_channel_vs_private_message(self):
        ...
        for server_name, server_subs in all_subs.items():
            if "#joensuu" in server_subs and "varoitukset" in server_subs["#joensuu"]:
                ...
                subscriptions.toggle_subscription(
                    "#joensuu", server_name, "varoitukset"
                )
                break
        ...
        self.assertTrue(
            responses, "Should have gotten a response for private message command"
        )
        response = responses[0]
        ...
        self.assertTrue(
            user_subscribed,
            f"User should be subscribed somewhere, got: {all_subs_after_pm}",
        )
