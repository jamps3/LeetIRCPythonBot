...
def test_mega_leet_detection():
    ...
    for timestamp in test_cases:
        ...
        if timestamp == "13:37:13.371337133":
            ...
            assert level == 'ultimate', f"Perfect timestamp should be ultimate, got '{level}'"
        elif result['total_count'] >= 3:
            assert level == 'mega', f"Expected 'mega' for {result['total_count']} occurrences, got '{level}'"
...
def test_super_leet_detection():
    ...
    for timestamp in test_cases:
        ...
        if result['total_count'] == 2:
            assert level == 'super', f"Expected 'super' for 2 occurrences, got '{level}'"
            ...
            assert level in ['leet', 'nano'], f"Expected 'leet' or 'nano' for 1 occurrence, got '{level}'"
...
def test_nano_leet_detection():
    ...
    for timestamp in test_cases:
        ...
        if result['nano_count'] > 0 and result['time_count'] == 0:
            assert level == 'nano', f"Expected 'nano' for nanosecond-only 1337, got '{level}'"
...
def test_check_message_for_leet():
    ...
    for nick, timestamp, expected_level in test_cases:
        ...
        if expected_level:
            ...
            assert timestamp in message, f"Message should contain timestamp '{timestamp}'"
            print(f"    Achievement: {message}")
        else:
            assert result is None, f"Expected no achievement for {timestamp}, got {result}"
...
def test_achievement_stats():
    ...
    for level in expected_levels:
        ...
        print(f"    {level}: {achievement['emoji']} {achievement['name']} - {achievement['criteria']}")
...
def test_timestamp_generation():
    ...
    for i in range(5):
        ...
        assert len(timestamp) == 18, f"Timestamp should be 18 chars, got {len(timestamp)}"
        ...
        assert len(nano_part) == 9, f"Nanosecond part should be 9 digits, got {len(nano_part)}"
