...
def test_api_key_loading():
    ...
    try:
        ...
        print(f"YOUTUBE_API_KEY: {'Found' if youtube_key else 'Not found'} (length: {len(youtube_key) if youtube_key else 0})")
        
        # Test other API keys
        weather_key = get_api_key("WEATHER_API_KEY")
        print(f"WEATHER_API_KEY: {'Found' if weather_key else 'Not found'} (length: {len(weather_key) if weather_key else 0})")
        
        electricity_key = get_api_key("ELECTRICITY_API_KEY")
        print(f"ELECTRICITY_API_KEY: {'Found' if electricity_key else 'Not found'} (length: {len(electricity_key) if electricity_key else 0})")
        
        openai_key = get_api_key("OPENAI_API_KEY")
        print(f"OPENAI_API_KEY: {'Found' if openai_key else 'Not found'} (length: {len(openai_key) if openai_key else 0})")
