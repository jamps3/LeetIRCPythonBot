...
class IPFSService:
    ...
    def __init__(self):
        ...
        if not self.ipfs_available:
            self.logger.warning("IPFS daemon not available. IPFS commands will be disabled.")
    ...
    def _download_file(self, url: str, max_size: int) -> Tuple[Optional[str], Optional[str], int]:
        """
        ...
        try:
            ...
            if content_length:
                ...
                if file_size > max_size:
                    return None, f"File too large: {file_size} bytes (max: {max_size} bytes)", file_size
            ...
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    ...
                    if downloaded_size > max_size:
                        ...
                        return None, f"File too large: {downloaded_size} bytes (max: {max_size} bytes)", downloaded_size
    ...
    def add_file_from_url(
        ...
        max_size = float('inf') if has_valid_password else self.max_size_without_password
        
        self.logger.info(f"Adding file from URL: {url} (password: {'yes' if has_valid_password else 'no'})")
    ...
    def get_ipfs_info(self, ipfs_hash: str) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                'success': False,
                'message': f"Error getting IPFS info: {str(e)}"
            }
