...
class IRCMessage:
    ...
    def is_private_message(self) -> bool:
        ...
        return self.type == IRCMessageType.PRIVMSG and self.target and not self.target.startswith('#')
    
    @property
    def is_channel_message(self) -> bool:
        """Check if this is a channel message."""
        return self.type == IRCMessageType.PRIVMSG and self.target and self.target.startswith('#')
...
class IRCClient:
    ...
    def __init__(self, server_config: ServerConfig, nickname: str, 
        ...
        # Message handlers
        self._message_handlers: Dict[IRCMessageType, List[Callable[[IRCMessage], None]]] = {}
    ...
    def add_message_handler(self, message_type: IRCMessageType, 
                           handler: Callable[[IRCMessage], None]):
    ...
    def remove_message_handler(self, message_type: IRCMessageType, 
                              handler: Callable[[IRCMessage], None]):
    ...
    def connect(self) -> bool:
        ...
        """
        try:
            self.log(f"Connecting to {self.server_config.host}:{self.server_config.port}...", "INFO")
    ...
    def _send_raw(self, message: str):
        ...
        if not self.socket or self.connection_info.state == IRCConnectionState.DISCONNECTED:
    ...
    def read_messages(self) -> List[IRCMessage]:
        ...
        """
        if not self.socket or self.connection_info.state != IRCConnectionState.CONNECTED:
            return []
    ...
    def run_forever(self, stop_event: Optional[threading.Event] = None):
        ...
        try:
            while not stop_event.is_set() and self.connection_info.state == IRCConnectionState.CONNECTED:
                try:
                    self.read_messages()
...
def create_irc_client(server_name: str = "SERVER1", 
                     nickname: str = None,
                     log_callback: Optional[Callable[[str, str], None]] = None) -> IRCClient:
    """
