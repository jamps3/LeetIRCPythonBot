...
class TestScheduledMessages(unittest.TestCase):
    ...
    def test_schedule_message(self):
        ...
        message_id = service.schedule_message(
            mock_irc, "#test", "Test message", 
            target_time.hour, target_time.minute, target_time.second, 0
        )
...
class TestIPFSService(unittest.TestCase):
    ...
    def test_download_file_size_check(self, mock_get, mock_head):
        ...
        temp_file, error, size = service._download_file("http://example.com/large.file", 100000000)  # 100MB limit
...
class TestEurojackpotService(unittest.TestCase):
    ...
    def test_last_results(self, mock_get):
        ...
        mock_response.json.return_value = {
            ...
            'results': {
                'primary': [7, 14, 21, 28, 35],
                'secondary': [3, 8]
            },
            'prizeBreakdown': [
                {'prizeRank': 1, 'winners': 2}
            ]
        }
        mock_get.return_value = mock_response
    ...
    def test_convenience_functions(self):
        ...
        from services.eurojackpot_service import get_eurojackpot_numbers, get_eurojackpot_results
...
def run_tests():
    ...
    if result.wasSuccessful():
        ...
        print("  2. 📁 IPFS Integration - File sharing with size limits and admin override")
