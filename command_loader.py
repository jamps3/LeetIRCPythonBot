...
from command_registry import (
    ...
    CommandContext, CommandResponse, CommandType, CommandScope
)
...
async def process_irc_command(message: str, sender: str, target: str, 
                            irc_connection, bot_functions: Dict[str, Any]) -> bool:
    """
    ...
    # Process the command
    try:
        response = await process_command_message(message, context, bot_functions_with_irc)
            ...
            if response.should_respond and response.message:
                ...
                if notice_message:
                    if response.split_long_messages:
                        ...
                        if split_func:
                            parts = split_func(response.message, 400)  # IRC message limit
                            for part in parts:
...
async def process_console_command_new(command_text: str, bot_functions: Dict[str, Any]) -> bool:
    """
...
def enhanced_process_console_command(command_text: str, bot_functions: Dict[str, Any]):
    """
    Enhanced console command processor that tries the new system first,
...
def enhanced_process_irc_message(irc, message, bot_functions):
    ...
    try:
        ...
        log_func = bot_functions.get('log')
        if log_func:
            log_func(f"New command system failed for IRC command '{text}': {e}", "WARNING")
